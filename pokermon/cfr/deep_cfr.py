...
from pokermon.cfr.traversal import KUHN_FEATURE_DIM, traverse_kuhn
...
class DeepCFR:
    ...
    def train_kuhn(
        ...
        for t in iterator:
            ...
            for p in range(self.num_players):
                ...
                if self.advantage_memories[p].size > self.batch_size:
                    self.advantage_nets[p] = self._make_network(
                        self.feature_dim, self.hidden_dim, self.num_actions, 2
                    )
            ...
            if verbose and isinstance(iterator, trange):
